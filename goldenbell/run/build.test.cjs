...
test('production worker smoke: routes, all nested modules/fonts/MIME and local-only dependency graph', async () => {
  ...
  for (const name of ['index.html','goldenbell.css','reveal.js','hero-bell.png','fonts/DNFBitBit-Regular.woff2','fonts/DNFBitBit-LICENSE.txt']) {
    ...
    if(name.endsWith('.js')) assert.equal(response.headers.get('content-type'),'text/javascript; charset=utf-8');
