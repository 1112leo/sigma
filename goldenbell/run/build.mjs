...
// carries the same three files in landing/ so its build remains self-contained.
const landingRoot = existsSync(join(root, 'landing/index.html')) ? join(root, 'landing') : join(root, '..');
for (const file of ['index.html', 'goldenbell.css', 'hero-bell.png']) {
  const name = `landing/${file}`;
  assets[name] = (await readFile(join(landingRoot, file))).toString('base64');
  contentTypes[name] = { '.html': 'text/html; charset=utf-8', '.css': 'text/css; charset=utf-8', '.png': 'image/png' }[extname(file)];
}
